...
제 목표는 언젠가 Anti-Cheat Engineer로 활동하며,  
개발자들이 애써 만든 게임이 부정 행위로 망가지는 것을 막는 것입니다.  

강력한 보안 시스템을 설계하고, 혁신적인 방법으로 치팅을 차단하여  
전 세계의 게이머들이 공정한 환경에서 게임을 즐길 수 있도록 돕고 싶습니다.

게임과 기술에 대한 저의 사랑이 이 모든 신념의 근원입니다.  
그 사랑을 바탕으로 더 나은 보안 기술을 제공하고, 게임 커뮤니티에 기여하는 것이 제 꿈입니다.

---
관심분야

📡 ESP (Extra Sensory Perception)
개요: 게임 내에서 플레이어의 위치, 아이템 등을 비정상적으로 표시하는 기능. 흔히 치트 프로그램에서 사용됨.
...
대응 방안: 화면 외적 오버레이 탐지 기술 및 비인가된 화면 데이터 변경 탐지.

🎯 AIMBOT (자동 조준 프로그램)
개요: 치트 프로그램의 일종으로, 타겟을 자동으로 조준하여 불공정한 이점을 제공.

연구 내용:

마우스 이벤트 및 좌표 기반의 자동화 코드 탐지.

이미지/픽셀 기반 조준 알고리즘 분석 및 차단.

대응 방안: 인공지능(AI)을 활용한 비정상적인 마우스 움직임 패턴 분석.

🛠️ 메모리변조 (Memory Modification)
개요: 게임의 메모리 값을 변조하여 무한 자원, 데미지 증폭 등의 효과를 구현.
...
대응 방안: ASLR(Address Space Layout Randomization) 및 메모리 암호화 기술 적용.

🌐 서버정보탈취 (Server Information Leakage)
개요: 게임 서버와 클라이언트 간 통신 데이터를 가로채 서버 구조와 정보를 분석.
...
대응 방안: 패킷 분석 방어 및 Secure Socket Layer (SSL/TLS) 강화.

🔄 리버싱 (Reversing)
개요: 소프트웨어 구조를 역분석하여 치트 개발 및 악성코드 주입에 활용.
...
대응 방안: Anti-Debugging 기술 및 실행 시간 변조 탐지.

🕵️ 기드라 (Ghidra)
개요: 미국 NSA가 개발한 오픈소스 리버스 엔지니어링 툴. 악성코드 분석 및 보안 취약점 연구에 활용.
...
응용 방안: 효율적인 코드 분석을 통한 치트 탐지 및 취약점 수정.
